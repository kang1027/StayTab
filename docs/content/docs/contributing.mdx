...
Xcode 16+ and the macOS 26 SDK are required. The Liquid Glass code paths are gated
...
They cover **pure logic only** — switcher metrics, row labels, catalog filtering,
fuzzy match, updater parsing, Liquid Glass selection, settings portability. UI
behaviour is verified by hand: the switcher needs a live WindowServer plus
Accessibility, so that surface fails headless and is not part of the unit run.
...
5. Surface it in the right settings pane. Anything fragile or new goes behind the
   off-by-default **Experimental** pane.
