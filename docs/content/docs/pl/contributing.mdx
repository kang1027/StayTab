...
Wymagane są Xcode 16+ i SDK macOS 26. Ścieżki kodu Liquid Glass są ograniczone do
...
Obejmują **wyłącznie czystą logikę** — metryki przełącznika, etykiety wierszy,
filtrowanie katalogu, dopasowanie rozmyte, analizę aktualizacji, wybór Liquid Glass i
przenośność ustawień. Zachowanie UI jest sprawdzane ręcznie: przełącznik potrzebuje
działającego WindowServer i uprawnienia Dostępność, dlatego ten obszar nie działa w
środowisku bez interfejsu graficznego i nie należy do testów jednostkowych.
...
5. Udostępnij ją we właściwym panelu ustawień. Wszystko, co niestabilne lub nowe,
   trafia do domyślnie wyłączonego panelu **Eksperymentalne**.
