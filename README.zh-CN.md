...
- **深入标签页** — 在窗口含有标签页的行上按 `\`，即可选择特定标签页（Safari、Chrome、Arc、Brave、Edge、Vivaldi、Opera、Dia、Finder、Terminal、iTerm）。
- **将标签页显示为行** — 可将每个原生或浏览器标签页直接显示为独立行，而不只是藏在 `\` 预览中；还提供实验性的标签页最近使用顺序，并在 Safari/Chrome 需要自动化权限时给出明确提示。
...
- **最小化与隐藏** — 包含最小化窗口、隐藏的 App 和无窗口 App。
