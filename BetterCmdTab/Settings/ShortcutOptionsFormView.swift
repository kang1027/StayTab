...
final class ShortcutOptionsFormView: NSView {
    ...
    private func buildCards() {
        ...
        addIntRow(to: appearance, title: String(localized: "Corner radius"), values: [-1, 0, 5, 10, 15, 20, 25, 30, 35, 40], display: { $0 < 0 ? String(localized: "Square") : ($0 == 0 ? String(localized: "Automatic") : "\($0) pt") }, current: override.panelCornerRadius) { [weak self] in self?.override.panelCornerRadius = $0; self?.persist() }
        ...
        addBoolRow(to: appearance, title: String(localized: "Show status icons"), current: override.showWindowStatusIcons) { [weak self] in self?.override.showWindowStatusIcons = $0; self?.persist() }
